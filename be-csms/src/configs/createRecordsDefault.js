...
module.exports = {
    ...
    createRecordsDefault: async () => {
        try {
            ...
            if (state1 == null) await Order_State.create({ state_id: 1, state_name: 'Waiting for Confirmation' })

            state2 = await Order_State.findOne({ where: { state_id: 2 } })
            if (state2 == null) await Order_State.create({ state_id: 2, state_name: 'Confirmed' })

            state3 = await Order_State.findOne({ where: { state_id: 3 } })
            if (state3 == null) await Order_State.create({ state_id: 3, state_name: 'In Shipping' })

            state4 = await Order_State.findOne({ where: { state_id: 4 } })
            if (state4 == null) await Order_State.create({ state_id: 4, state_name: 'Delivered' })

            state5 = await Order_State.findOne({ where: { state_id: 5 } })
            if (state5 == null) await Order_State.create({ state_id: 5, state_name: 'Canceled' })

            state6 = await Order_State.findOne({ where: { state_id: 6 } })
            if (state6 == null) await Order_State.create({ state_id: 6, state_name: 'Cancel By Shop' })
        } catch (err) {
            console.log(err)
        }
    }
